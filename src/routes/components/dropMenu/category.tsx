...
export const DropMenuCategory = () => {
  const model = useMemo(() => {
    return CATEGORIES.reduce((acc: MenuModelItemProps[], item) => {
      acc.push({
        id: item.id,
        render: (options: RenderOptionProps) => (
          <CategoryMenuItem key={item.id} {...options}>
            {item.name}
          </CategoryMenuItem>
        ),
      });
...
export const Route = createFileRoute('/components/dropMenu/category')({
  component: () => <DropMenuCategory />,
  staticData: {
    title: 'DropMenu. Пример с группами',
    description: 'Пример с группировкой опций по категориям.',
  },
});
