    ...
    <ExampleSection
      text={
        <>
          ...
          <PStyled>
            Для того чтобы создать фильтр для столбца достаточно задать для него параметр renderFilter - функцию,
            которая будет отрисовывать содержимое меню фильтра. Данная функция имеет в качестве входных параметров
            колбек closeMenu, при вызове которого происходит закрытие меню фильтра, и объект column, через метод
            setFilterValue которого можно управлять значением фильтрации колонки.
          </PStyled>
          <PStyled>
            Меню фильтра является произвольным и полностью контролируется пользователем. Закрытие меню и установка
            значения фильтра производятся пользователем внутри функции renderFilter. С помощью параметров
            filterMenuAlignSelf, filterMenuClassName, filterMenuCssMixin и filterMenuStyle пользователь также может
            управлять выравниваем фильтра, добавлять className и изменять его стили.
          </PStyled>
          <PStyled>
