...
export const TreeWithControlCheckCount = () => {
  ...
  return (
    <>
      <ExampleSection text="Пример показывает обработку состояния checked в модели. Checked являются узлы, которые непосредственно выбраны, либо узлы, у которых все дочерние элементы находятся в состоянии checked">
        <T font="Subtitle/Subtitle 1" as="p">
          Выбрано элементов: {countSelected}
        </T>
        <Tree dimension="s" model={dataList} onChange={changeTreeValues} />
      </ExampleSection>
    </>
  );
};
