...
import { Table, T, Button, DateField, RadioButton, FieldSet, OrderedList, ListItem } from '@admiral-ds/react-ui';
...
export const TableFilter = () => {
  ...
  const [columns, setCols] = useState([...columnList]);
  const [numFilterActive, setNumFilterActive] = useState(false);
      ...
      text={
        <>
          ...
          <PStyled>
            ...
            функцию, которая будет отрисовывать содержимое меню фильтра. Данная функция имеет в качестве входных
            параметров объект столбца и объект с двумя свойствами:
          </PStyled>
          <PStyled as="div">
            <OrderedList dimension="s">
              ...
              <ListItem>
                <code>setFilterActive</code> - колбек, который устанавливает фильтр в активное/неактивное состояние.
                Колбек <code>setFilterActive</code> - это устаревшее api, вместо него рекомендуется использовать
                параметр столбца <code>isFilterActive</code>, который также устанавливает фильтр в активное/неактивное
                состояние. В неактивном состоянии иконка фильтра окрашена в серый цвет, при активном (включенном
                фильтре) иконка фильтра окрашивается в синий цвет.
              </ListItem>
            </OrderedList>
          </PStyled>
          <PStyled>
            Меню фильтра является произвольным и полностью контролируется пользователем. Закрытие меню и установка
            фильтра в активное/неактивное состояние производится пользователем внутри функции <code>renderFilter</code>{' '}
            с помощью вышеописанных колбеков. С помощью параметров <code>filterMenuAlignSelf</code>,{' '}
            <code>filterMenuClassName</code>, <code>filterMenuCssMixin</code> и <code>filterMenuStyle</code>{' '}
            пользователь также может управлять выравниваем фильтра, добавлять <code>className</code> и изменять его
            стили.
          </PStyled>
          <PStyled>
          ...
          <PStyled>
            ...
            которые будут срабатывать соответственно при открытии и закрытии меню фильтра. А также для столбца можно
            задать колбек <code>onFilterMenuClickOutside</code>, который будет срабатывать при клике вне меню фильтра.
            Данный колбек имеет в качестве входных параметров объект с двумя свойствами (<code>closeMenu</code>,{' '}
          ...
          <PStyled>
            ...
            открывается дропдаун с календарем) убедитесь, что компоненты библиотеки обернуты в{' '}
            <code>DropdownProvider</code>. Пример использования <code>DropdownProvider</code> есть в <code>readme</code>
            .
          </PStyled>
        </>
      }
