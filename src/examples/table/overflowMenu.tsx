...
import { Table, T, TooltipHoc, RowAction, UnorderedList, ListItem, MenuItem, OverflowMenu } from '@admiral-ds/react-ui';
import type { Column, TableRow, RenderOptionProps } from '@admiral-ds/react-ui';
...
export const TableOverflowMenu = () => {
  const [cols, setCols] = useState([...columnList]);
  const [cols2, setCols2] = useState([...columnList]);
  ...
  return (
    <ExampleSection
      text={
        <>
          <PStyled>
            Опционально со строками можно производить набор действий через <code>OverflowMenu</code>, которое по
            умолчанию появляется при ховере над строкой. При этом, чтобы не накладываться на возможный текст, под
            иконкой меню добавляется квадратная подложка белого цвета. В примере ниже <code>OverflowMenu</code> задано
            для первых двух строк в таблицах.
          </PStyled>
          <PStyled>
            Для того чтобы задать для строки <code>OverflowMenu</code> необходимо для строки прописать функцию{' '}
            <code>overflowMenuRender</code>. Входные параметры функции: сама строка и колбек{' '}
            <code>onVisibilityChange</code>. Колбек необходимо вызывать при открытии/закрытии меню для того, чтобы
            таблица могла управлять видимостью <code>OverflowMenu</code>. В качестве результата функция должна
            возвращать компонент <code>OverflowMenu</code>. Размер <code>OverflowMenu</code> следует задавать согласно
            правилу:
          </PStyled>
          <PStyled as="div">
          ...
          <PStyled>
            ...
            использовать функцию <code>actionRender</code> (в примере используется для 3-4 строк). На вход функция
            получает саму строку, а возвращает компонент <code>RowAction</code> (экспортируется из библиотеки), внутрь
            которого необходимо передать иконку для обозначения действия над строкой.
          </PStyled>
          <PStyled>
            Опционально допускается, чтобы <code>OverflowMenu</code> и иконки одиночных действий были видны постоянно, а
            не только по ховеру. Данное поведение можно задать с помощью параметра <code>showRowsActions</code>. Если{' '}
            <code>showRowsActions=true</code>, то все иконки меню и иконки одиночных действий во всех строках таблицы
            отображаются постоянно. При этом в строки, для которых не заданы действия, добавляется подложка, для того
            чтобы визуально был выделен столбец с действиями над строками. Начиная с версии 8.45.0, при{' '}
    ...
    >
      ...
      <Separator />
      <Table dimension="m" rowList={rowList} columnList={cols} onColumnResize={handleResize} />
      <Separator style={{ height: '40px' }} />
      <T font="Body/Body 2 Long">Пример с постоянно видимыми иконками действий над строками</T>
      <Separator />
      <Table dimension="m" rowList={rowList} columnList={cols2} onColumnResize={handleResize2} showRowsActions />
    </ExampleSection>
  );
};
