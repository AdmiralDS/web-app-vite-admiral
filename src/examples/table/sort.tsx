  ...
  return (
    <ExampleSection
      text={
        <>
          <PStyled>
            По умолчанию возможность сортировки столбца отключена. Чтобы сделать столбец сортируемым, необходимо задать
            ему параметр <code>sortable:true</code>. Сортировка - это контролируемый пользователем параметр. Чтобы
            включить для столбца сортировку по возрастанию/убыванию, пользователь должен задать для столбца параметр{' '}
            <code>sort:'asc'|'desc'</code>. Если для столбца задан только параметр <code>sortable:true</code>, а
            параметр <code>sort</code> не задан, это говорит о том, что столбец сортируемый (при наведении на его
            заголовок будет видна стрелка сортировки), но в данный момент к нему никакая сортировка не применена.
          </PStyled>
          <PStyled>
          ...
          <PStyled>
            При изменении сортировки столбца будет срабатывать колбек <code>onSortChange</code>, который будет
            возвращать объект со свойствами: <code>name</code> - уникальное имя столбца, к которому была применена
            сортировка, <code>sort</code> - тип сортировки ('asc' - возрастающая, 'desc' - убывающая и 'initial' -
            отмена сортировки, возврат к первоначальному состоянию). Сортировка массива строк происходит на стороне
            пользователя при срабатывании колбека <code>onSortChange</code>. Для столбца таблицы можно задать параметр{' '}
            <code>type</code>, на основе которого для столбцов одинакового типа можно задать единые функции сортировки.
          </PStyled>
          <PStyled>
    ...
    >
      <Table columnList={cols} rowList={rows} onSortChange={handleSort} onColumnResize={handleResize} />
    </ExampleSection>
  );
};
