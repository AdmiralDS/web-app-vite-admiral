    ...
    <ExampleSection
      text={
        <>
          ...
          <PStyled>
            Строки таблицы не ограничены по высоте. В ячейке строки можно отрисовать любой <code>ReactNode</code>,
            передав его в <code>rowList</code>. Также с помощью функции <code>renderCell</code> можно переопределить
            стандартный вид ячеек, относящихся к определенному столбцу таблицы (смотрите "Пример кастомизации компонента
            ячейки").
          </PStyled>
        </>
      }
    >
      <Table rowList={rowList} columnList={cols} headerLineClamp={2} onColumnResize={handleResize} />
    </ExampleSection>
  );
};
