...
export const TableRowDragDrop = () => {
  const [cols, setCols] = useState(columnList);
  const [cols2, setCols2] = useState(columnList2);
  const [rows, setRows] = useState(rowList);
  const [rows2, setRows2] = useState(rowList2);
  ...
  return (
    <ExampleSection
      text={
        <>
          <PStyled>
            Функция изменения порядка строк является опциональной. По умолчанию строки таблицы не подлежат перемещению.
            Для того чтобы строки можно было перемещать, необходимо в таблицу передать параметр{' '}
            <code>rowsDraggable</code> равный <code>true</code>. При установленном параметре <code>rowsDraggable</code>{' '}
            у каждой строки (кроме строк с заголовками групп) слева появится иконка с 6ю точками, так называемая{' '}
            <code>Drag</code> зона.
          </PStyled>
          <PStyled>
            Для перемещения строки следует мышью навестись на <code>Drag</code> зону и “зажать” левую кнопку мыши над
            ней, после чего над строкой появится миниатюра строки, в которой отображается содержание первой ячейки
            строки. Передвигая данную миниатюру по вертикали в пределах тела таблицы, можно изменить местоположение
            интересующей строки. При этом, отменяется сортировка строк, если она была произведена ранее. Если
            перемещаемая миниатюра выходит за границы тела таблицы наполовину своей высоты, то перемещение становится
            невозможным, курсор принимает соответствующий вид.
          </PStyled>
          <PStyled>
            Процесс перемещения строк контролируется пользователем. Поэтому для таблицы должен быть задан колбек{' '}
            <code>onRowDrag</code>, который срабатывает при каждой попытке изменить местоположение строки таблицы.
            Данный колбек имеет три параметра:
          </PStyled>
          <PStyled as="div">
            <UnorderedList dimension="s">
              ...
              <ListItem>
                ...
                <code>nextRowId</code> может быть также равен <code>null</code>, если строка пытается встать в самый
                конец таблицы;
              </ListItem>
              <ListItem>
                <code>groupRowId - id</code> групповой строки (строки с заголовком группы), по данному <code>id</code>{' '}
                можно определить к какой группе будет относиться перемещаемая строка. Параметр <code>groupRowId</code>{' '}
                может быть также равен <code>null</code>, это значит, что перемещаемая строка не будет относиться ни к
                какой группе.
              </ListItem>
            </UnorderedList>
          </PStyled>
          <PStyled>
            При срабатывании колбека <code>onRowDrag</code>, пользователь должен будет соответственно обновить список
            строк (<code>rowList</code>) для таблицы.
          </PStyled>
          <PStyled>
          ...
          <PStyled as="div">
            <OrderedList dimension="s">
              ...
              <ListItem>
                задать для компонента <code>DropdownProvider</code> параметр <code>rootRef</code>, где{' '}
                <code>rootRef</code> - это реф на <code>dom</code>-элемент, внутри которого отрендерится миниатюра
                строки.
              </ListItem>
            </OrderedList>
          </PStyled>
