    ...
    <ExampleSection
      text={
        <PStyled>
          ...
          <code>displayRowExpansionColumn</code>. Стрелка позволяет развернуть строку и посмотреть более
          детализированную информацию о строке. По нажатию на любую из стрелок срабатывает колбек{' '}
          <code>onRowExpansionChange</code>. Развернутое/свернутое состояние строки задается параметром{' '}
          <code>expanded</code>. А с помощью функции <code>expandedRowRender</code> происходит рендер развернутой части
          строки (рендер детализированной информации).
        </PStyled>
      }
    >
      <Table
